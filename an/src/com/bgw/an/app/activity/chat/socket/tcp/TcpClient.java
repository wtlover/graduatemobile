package com.bgw.an.app.activity.chat.socket.tcp;
...
import com.bgw.an.app.activity.chat.file.Constant;
import com.bgw.an.app.activity.chat.file.FileState;
import com.bgw.an.app.activity.chat.file.FileStyle;
...
public class TcpClient implements Runnable {
	...
	public void sendFile(ArrayList<FileStyle> fileStyles,
			ArrayList<FileState> fileStates, String target_IP) {
	...
	public class SendFileThread extends Thread {
		...
		public void sendFile() {
			...
			try {
				...
				dataOutput.writeUTF(filePath.substring(filePath
						.lastIndexOf(File.separator) + 1)
						+ "!"
						+ fileSize
						+ "!" + SessionUtils.getIMEI() + "!" + type);
				int count = 0;
				long length = 0;
				...
				case IMAGE:
					...
					imageMsg.setMsgContent(FileUtils.getNameByPath(imageMsg
							.getMsgContent()));
				...
				case VOICE:
					Message voiceMsg = new Message(SessionUtils.getIMEI(),
							DateUtils.getNowtime(), fs.fileName, type);
					voiceMsg.setMsgContent(FileUtils.getNameByPath(voiceMsg
							.getMsgContent()));
					UDPSocketThread.sendUDPdata(IPMSGConst.IPMSG_SENDMSG,
							target_IP, voiceMsg);
