...
public class ChatActivity extends BaseMessageActivity implements
	...
	private void init() {
		...
		mHeaderLayout.setTitleChat(
				ImageUtils.getIDfromDrawable(this,
		...
		mClvList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				case IMAGE:
					Intent imgIntent = new Intent(ChatActivity.this,
							ImageBrowserActivity.class);
					imgIntent.putExtra(ImageBrowserActivity.IMAGE_TYPE,
							ImageBrowserActivity.TYPE_PHOTO);
					imgIntent.putExtra(ImageBrowserActivity.PATH,
							msg.getMsgContent());
					ChatActivity.this.startActivity(imgIntent);
//					((ChatActivity) getApplicationContext()).overridePendingTransition(
//							R.anim.zoom_enter, 0);
				...
				if (cursor != null) {
					...
					if (cursor.getCount() > 0 && cursor.moveToFirst()) {
						...
						if (ImageUtils.bitmapIsLarge(bitmap)) {
							...
							if (path != null) {
								ImageUtils.createThumbnail(this, path,
										THUMBNAIL_PATH + File.separator);
			...
			if (resultCode == RESULT_OK) {
				if (mCameraImagePath != null) {
					mCameraImagePath = ImageUtils.savePhotoToSDCard(
							ImageUtils.CompressionPhoto(mScreenWidth,
									mCameraImagePath, 2),
	...
	private void startRecord() {
		...
		RECORD_FILENAME = System.currentTimeMillis()
				+ com.bgw.an.app.activity.chat.util.TextUtils
						.getRandomNumStr(3);
