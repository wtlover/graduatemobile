...
public class UDPSocketThread implements Runnable {
	...
	public void processCommand(int commandNo, IPMSGProtocol ipmsgRes,
			String senderIMEI, String senderIp) {
		...
		case IPMSGConst.IPMSG_SENDMSG: {
			...
			case IMAGE:
				...
				msg.setMsgContent(BaseApplication.IMAG_PATH + File.separator
						+ msg.getSenderIMEI() + File.separator
						+ msg.getMsgContent());
				String THUMBNAIL_PATH = BaseApplication.THUMBNAIL_PATH
						+ File.separator + msg.getSenderIMEI();

				LogUtils.d(TAG, "缩略图路径:" + THUMBNAIL_PATH);
				LogUtils.d(TAG, "图片接收路径:" + msg.getMsgContent());

				ImageUtils.createThumbnail(mContext, msg.getMsgContent(),
	...
	public static synchronized void sendUDPdata(int commandNo,
			InetAddress targetIP) {
		sendUDPdata(commandNo, targetIP, null);
	}

	public static synchronized void sendUDPdata(int commandNo,
			InetAddress targetIP, Object addData) {
	...
	public static synchronized void sendUDPdata(
			final IPMSGProtocol ipmsgProtocol, final String targetIP) {
		// 构造发送报文
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					sendBuffer = ipmsgProtocol.getProtocolJSON()
							.getBytes("gbk");
