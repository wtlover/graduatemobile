...
import com.bgw.an.app.activity.chat.entity.Message.CONTENT_TYPE;
...
public class SqlDBOperate {
	...
	public int getIDByIMEI(String imei) {
		Cursor cursor = userDataBase.query(userSQLHelper.getTableName(),
				new String[] { "id" }, "IMEI=?", new String[] { imei }, null,
				null, null);
	...
	public UserInfo getUserInfoByID(int id) {
		...
		// orderBy)
		Cursor cursor = userDataBase.query(userSQLHelper.getTableName(),
				new String[] { "id", "name", "age", "IMEI", "sex", "ip",
						"status", "avater", "lastdate", "device",
						"constellation" }, "id=?",
				new String[] { String.valueOf(id) }, null, null, null);
		if (cursor.moveToNext()) {
			UserInfo userInfo = new UserInfo(cursor.getInt(cursor
					...
					.getColumnIndex("IMEI")), cursor.getString(cursor
					.getColumnIndex("ip")), cursor.getInt(cursor
	...
	public UserInfo getUserInfoByIMEI(String imei) {
		...
		Cursor cursor = userDataBase.query(userSQLHelper.getTableName(),
				new String[] { "id", "name", "age", "IMEI", "sex", "ip",
						"status", "avater", "lastdate", "device",
						"constellation" }, "IMEI=?", new String[] { imei },
				null, null, null);
		if (cursor.moveToNext()) {
			UserInfo userInfo = new UserInfo(cursor.getInt(cursor
					.getColumnIndex("id")), cursor.getString(cursor
	...
	public void deteleUserInfo(Integer... ids) {
		if (ids.length > 0) {
			...
			// db = helper.getWritableDatabase();
			userDataBase.delete(userSQLHelper.getTableName(), "id in (" + sb
					+ ")", strPid);
		}
	}
	...
	public List<UserInfo> getScrollDataOfUserInfo(int start, int count) {
		...
		// db = helper.getWritableDatabase();
		Cursor cursor = userDataBase.query(userSQLHelper.getTableName(),
				new String[] { "id", "name", "age", "sex", "IMEI", "ip",
						"status", "avater", "lastdate", "device",
						"constellation" }, null, null, null, null, "id desc",
				start + "," + count);
		while (cursor.moveToNext()) {
			users.add(new UserInfo(cursor.getInt(cursor.getColumnIndex("id")),
					cursor.getString(cursor.getColumnIndex("name")), cursor
							.getInt(cursor.getColumnIndex("age")), cursor
							.getString(cursor.getColumnIndex("sex")), cursor
							.getString(cursor.getColumnIndex("IMEI")), cursor
							.getString(cursor.getColumnIndex("ip")), cursor
							.getInt(cursor.getColumnIndex("status")), cursor
							.getInt(cursor.getColumnIndex("avater")), cursor
							.getString(cursor.getColumnIndex("lastdate")),
					cursor.getString(cursor.getColumnIndex("device")), cursor
							.getString(cursor.getColumnIndex("constellation"))));
		}
		cursor.close();
		return users;
	}
	...
	public List<Integer> getIDOfChattingInfo(int senderID, int receiverID) {
		...
		Cursor cursor = chatInfoDataBase.query(
				...
				new String[] { String.valueOf(senderID),
						String.valueOf(receiverID) }, null, null, null);
		while (cursor.moveToNext()) {
	 ...
	 */
	public List<ChattingInfo> getAllMessageFromChattingInfo(int sendID,
			int receiverID) {
		List<ChattingInfo> infos = new ArrayList<ChattingInfo>();
		Cursor cursor = chatInfoDataBase.query(
				chatInfoSQLHelper.getTableName(),
				new String[] { "id", "sendID", "receiverID", "chatting",
						"date", "style" },
				"sendID=? and receiverID=?",
				new String[] { String.valueOf(sendID),
						String.valueOf(receiverID) }, null, null, null);
		while (cursor.moveToNext()) {
			infos.add(new ChattingInfo(cursor.getInt(cursor
	 ...
	 * 参数：chattinginfo类 作用：用来添加聊天记录
	 */
	public void addChattingInfo(int senderID, int recieverID, String time,
	 ...
	 * 参数：chattinginfo类 作用：用来添加聊天记录
	 */
	public void addChattingInfo(String senderIMEI, String recieverIMEI,
			String time, String content, CONTENT_TYPE type) {
	...
	public void updateChattingInfo(ChattingInfo info) {
		...
		values.put("date", info.getDate());
		values.put("style", info.getStyle());
		chatInfoDataBase.update(chatInfoSQLHelper.getTableName(), values,
	...
	public ChattingInfo getChattingInfoByID(int id) {
		...
		// orderBy)
		Cursor cursor = chatInfoDataBase.query(
				chatInfoSQLHelper.getTableName(), new String[] { "id",
						"sendID", "receiverID", "chatting", "date", "style" },
				"id=?", new String[] { String.valueOf(id) }, null, null, null);
		if (cursor.moveToNext()) {
			ChattingInfo chattingInfo = new ChattingInfo(cursor.getInt(cursor
					.getColumnIndex("id")), cursor.getInt(cursor
					.getColumnIndex("sendID")), cursor.getInt(cursor
					.getColumnIndex("receiverID")), cursor.getString(cursor
					.getColumnIndex("date")), cursor.getString(cursor
	...
	public void deteleChattingInfo(Integer... ids) {
		if (ids.length > 0) {
			...
			sb.deleteCharAt(sb.length() - 1);
			// db = helper.getWritableDatabase();
			chatInfoDataBase.delete(chatInfoSQLHelper.getTableName(), "id in ("
					+ sb + ")", strPid);
		}
	}
	...
	public List<ChattingInfo> getScrollDataOfChattingInfo(int start, int count) {
		...
		while (cursor.moveToNext()) {
			info.add(new ChattingInfo(
					cursor.getInt(cursor.getColumnIndex("id")), cursor
							.getInt(cursor.getColumnIndex("sendID")), cursor
							.getInt(cursor.getColumnIndex("receiverID")),
					cursor.getString(cursor.getColumnIndex("date")), cursor
							.getString(cursor.getColumnIndex("chatting")),
	...
	public List<Message> getScrollMessageOfChattingInfo(int start, int count,
		...
		Cursor cursor = chatInfoDataBase.query(
				...
				new String[] { String.valueOf(senderID),
						String.valueOf(recieverID), String.valueOf(senderID),
						String.valueOf(recieverID) }, null, null, "id desc",
				start + "," + count);
		while (cursor.moveToNext()) {
			Message message = chattingInfoToMessage(new ChattingInfo(
	...
	public long getCountOfChattingInfo() {
		// db = helper.getWritableDatabase();
		Cursor cursor = chatInfoDataBase.query(
				chatInfoSQLHelper.getTableName(), new String[] { "count(*)" },
